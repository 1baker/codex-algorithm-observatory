...
export function sanitizeEvent(input) {
  ...
  const safe = {
    source: String(input.source || "codex"),
    ...(input.sourceEventId ? { sourceEventId: String(input.sourceEventId) } : {}),
    method,
...
function sanitizeRuntime(runtime) {
  ...
  const result = Object.fromEntries(allowed.filter((key) => typeof runtime[key] === "string").map((key) => [key, runtime[key].slice(0, 64)]));
